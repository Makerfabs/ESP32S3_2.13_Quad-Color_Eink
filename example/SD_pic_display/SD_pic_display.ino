...
// The accelerometer is configured for +/-8 g at 1 kHz.  A shake must create
// two peaks in a short time, then the cooldown prevents one shake changing
// several pictures.  Adjust SHAKE_PEAK_G for a lighter/heavier trigger.
constexpr float QMI8658A_ACCEL_G_PER_LSB = 8.0f / 32768.0f;
constexpr float SHAKE_PEAK_G = 0.80f;
constexpr unsigned long IMU_SAMPLE_INTERVAL_MS = 25;
constexpr unsigned long SHAKE_PEAK_GAP_MIN_MS = 70;
constexpr unsigned long SHAKE_PEAK_WINDOW_MS = 450;
constexpr unsigned long SHAKE_COOLDOWN_MS = 2500;
...
constexpr size_t IMAGE_COUNT = sizeof(kImageFiles) / sizeof(kImageFiles[0]);
...
static bool initImu()
{
  ...
  Serial.println("QMI8658A ready: shake to show the next image");
  return true;
}
...
// Returns true once per deliberate shake.  We use acceleration polling here
// because it is easier to tune than the QMI8658A motion interrupt and works
// on every board revision; GPIO14 remains available for a future deep-sleep
// wake-on-motion implementation.
static bool shakeDetected()
{
  static unsigned long lastSampleMs = 0;
  static unsigned long firstPeakMs = 0;
  static unsigned long lastPeakMs = 0;
  static unsigned long lastShakeMs = 0;
  static bool gravityInitialized = false;
  static float filteredMagnitudeG = 1.0f;

  const unsigned long now = millis();
  if (now - lastSampleMs < IMU_SAMPLE_INTERVAL_MS) {
    return false;
  }
  lastSampleMs = now;
  ...
  if (!gravityInitialized) {
    filteredMagnitudeG = magnitudeG;
    gravityInitialized = true;
    return false;
  }

  const float dynamicAccelerationG = fabsf(magnitudeG - filteredMagnitudeG);
  // Low-pass estimate of gravity/orientation; it deliberately reacts much
  // slower than a shake, so a stationary tilted device does not trigger.
  filteredMagnitudeG = filteredMagnitudeG * 0.92f + magnitudeG * 0.08f;

  if (now - firstPeakMs > SHAKE_PEAK_WINDOW_MS) {
    firstPeakMs = 0;
  }
  if (dynamicAccelerationG < SHAKE_PEAK_G ||
      now - lastPeakMs < SHAKE_PEAK_GAP_MIN_MS) {
    return false;
  }
...
void loop()
{
  ...
  static bool firstImageShown = false;

  // Keep the first image visible after power-up.  Further refreshes happen
  // only after a deliberate shake, instead of the old fixed 8-second timer.
  if (!firstImageShown) {
    showImage(kImageFiles[imageIndex]);
